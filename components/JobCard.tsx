...
const JobCard = memo(function JobCard({ job, index = 0 }: JobCardProps) {
  return (
    <article className="fade-in" style={{ animationDelay: `${index * 0.05}s` }}>
      <Link 
        href={`/jobs/${job.slug}`} 
      ...
      >
        <div className="flex flex-col sm:flex-row sm:justify-between gap-4">
          <div className="flex gap-3 flex-1">
            ...
            <div>
              <h3 className="text-lg font-medium group-hover:text-emerald-600 transition-colors duration-200">
                {job.title} at <span className="text-emerald-600">{job.company}</span>
              </h3>
              <p className="text-slate-700 text-sm line-clamp-2 mt-1">{job.description}</p>
            </div>
          </div>
          <div className="space-y-3 sm:text-right">
            ...
            <time className="text-slate-400 text-xs" dateTime={job.postedAt.toISOString()}>
